...
public struct Specs: Codable {
  public init(title: String, date: Date, description: String?, tags: [String]) {
...
public extension Specs {
  init(fromPost post: WordpressPost) {
    self.init(title: post.title, date: post.date, description: post.meta["_yoast_wpseo_metadesc"], tags: post.tags)
  }
}
